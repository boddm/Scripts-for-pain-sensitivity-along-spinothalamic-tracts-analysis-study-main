...
S_mat = repmat(diag(S), 1, pls_opts.nPerms); % L×Nperm, expand actual singular values into matrix
sp = sum(Sp_vect >= S_mat, 2); % count for each LC
...
LC_pvals = (sp + 1) ./ (pls_opts.nPerms + 1); % add 1 to avoid zero, obtain p-value for each LC

%% ========== Display significant LC information ========== %%
