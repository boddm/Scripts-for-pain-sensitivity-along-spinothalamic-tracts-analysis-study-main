...
for iP = 1:pls_opts.nPerms
    ...
    % Keep X unchanged (no need to permute both X and Y)
    Xp = X; % X already normalized
    
    % Permute Y (within each group or across all subjects)
    Yp = myPLS_permuteY(Y, grouping, pls_opts.grouped_perm);
  
    % Compute cross-covariance between X and permuted Y
    Rp = myPLS_cov(Xp, Yp, grouping, pls_opts.grouped_PLS);
  
    % Singular value decomposition of Rp
    [Up, Sp, Vp] = svd(Rp, 'econ');
  
    % Procrustes rotation (correct for axis rotation/reflection)
    rotatemat = rri_bootprocrust(U, Up);
    Up = Up * Sp * rotatemat;
    Sp = sqrt(sum(Up.^2));
  
    % Store singular values for empirical null distribution
    Sp_vect(:, iP) = Sp';
end

%% ========== Display progress and newline ========== %%
if mod(iP, 200)
    fprintf('\n');
end
