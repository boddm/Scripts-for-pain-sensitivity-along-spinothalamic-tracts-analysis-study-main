...
% Initialize input, parameters, and save options for PLS analysis. Set default values, validate inputs, auto-fill missing fields, and ensure smooth downstream workflow. Supports multiple input formats, grouping, behavior/contrast/interaction analyses, normalization, plotting and saving parameters, etc.
%
% =============================
% Inputs:
...
%   pls_opts : structure of PLS analysis parameters; must include:
%       .behav_type           : type of behavioral analysis (behavior/contrast/contrastBehav/contrastBehavInteract)
%       .grouped_PLS          : perform group PLS (0/1)
%       .grouped_perm         : permute within groups (0/1)
%       .grouped_boot         : bootstrap within groups (0/1)
%       .boot_procrustes_mod  : bootstrap Procrustes rotation mode (1/2)
%       .save_boot_resampling : save bootstrap samples (0/1)
%       .normalization_img    : imaging data normalization (0–4, see myPLS_bootstrapping.m)
%       .normalization_behav  : behavioral data normalization (0–4, see myPLS_bootstrapping.m)
%   save_opts : structure of saving and plotting parameters; must include:
%       .prefix        : string, prefix for result filenames
%       .output_path   : string, output directory
%       .alpha         : significance level
%       .img_type      : image type (volume/corrMat/barPlot)
...
%       .grouped_plots : plot by groups (0/1)
%       .plot_boot_samples : plot bootstrap samples (0/1)
%
% =============================
% Outputs:
%   input     : structure, completed and standardized input data
%   pls_opts  : structure, completed and standardized PLS parameters
%   save_opts : structure, completed and standardized save/plot parameters
disp('... Initialization ...')
...
elseif exist(save_opts.output_path, 'dir')
    disp(['Removing and recreating output directory: ' save_opts.output_path])
    rmdir(save_opts.output_path, "s");
    mkdir(save_opts.output_path);
end
