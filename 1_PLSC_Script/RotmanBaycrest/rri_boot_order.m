function [boot_order, new_num_boot] = rri_boot_order(num_subj_lst, num_cond, num_boot, bscan, incl_seq, boot_type, from_nonstrat)
%% - from PLS toolbox
%
...
if strcmp(boot_type, 'nonstrat')
    ...
    %
    [tmp_order, new_num_boot] = rri_boot_order(total_subj, num_cond, ...
        num_boot, bscan, 0, 'strat', from_nonstrat);
    ...
    for r = 1:total_rows0
        for p = 1:size(tmp_order, 2)
            ...
            for g = 1:num_group
                if (ss > sum(num_subj_lst(1:(g-1))) && ss <= sum(num_subj_lst(1:g)))
                    break;
                end
            end
...
for g = 1:length(num_subj_lst)
    ...
    one_group = [];
    for p = 1:new_num_boot
        tmp = boot_order(:, [sum(num_subj_lst(1:(g-1)))+1:sum(num_subj_lst(1:g))], p);
