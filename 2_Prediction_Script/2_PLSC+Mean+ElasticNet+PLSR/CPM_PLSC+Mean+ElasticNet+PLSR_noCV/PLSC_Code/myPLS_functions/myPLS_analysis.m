...
% =============================
% Function:
% Main function to perform PLS analysis. Includes data normalization, % covariance matrix calculation,
...
%
% pls_opts : Structure, PLS analysis parameters, must contain following fields:
...
%                         0 = compute PLS for all subjects together
%                         1 = concatenate covariance matrices across groups (behavior PLS)
...
%                         0 = do not save
%                         1 = save
%   .behav_type          : String, behavior analysis type
%                         'behavior': standard behavior PLS (default)
%                         'contrast': compute only two-group contrast
...
%       .Lyb        : N×(L×number of groups)×P matrix, bootstrap behavior scores
%       .LC_img_loadings_boot : M×L×P matrix, bootstrap imaging loadings
%       .LC_behav_loadings_boot: B×(L×number of groups)×P matrix, bootstrap behavior loadings
...
% (If there are group contrasts in Y, groups will not be considered)
...
%% ICA convention: Convert LCs to have positive maximum value
for iLC = 1:nLC
    [~, maxID] = max(abs(V(:, iLC)));
...
%% ========== CPM pipeline does not require PLS scores, loadings, and permutation tests ========== %%
...
%% ========== Bootstrap test to determine PLS loading stability ========== %%
boot_results = myPLS_bootstrapping(X0, Y0, U, V, S, input.grouping, pls_opts); % Bootstrap test

%% ========== Aggregate all result variables into structure ========== %%
