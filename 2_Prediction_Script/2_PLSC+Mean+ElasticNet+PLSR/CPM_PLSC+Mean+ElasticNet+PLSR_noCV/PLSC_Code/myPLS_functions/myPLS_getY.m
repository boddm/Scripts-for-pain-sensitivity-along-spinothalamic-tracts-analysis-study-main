...
% Ensure names are row vectors for easy concatenation
...
%% ========== Generate Contrast Vector (if needed) ========== %%
% If analysis mode contains contrast, generate group contrast vector c and its name
if contains(behavMode, 'contrast')
    ...
    elseif nGroups == 3
        ...
        % Only support 2-group or 3-group contrast
        error('Only 2-group or 3-group contrast is supported');
    end
end
...
switch behavMode
    ...
    case 'contrastBehavInteract'
        ...
        % Concatenate contrast vectors and interaction terms
        Y0 = [c, Y0];
...
nDesignScores = size(Y0, 2); % Number of design variables
end