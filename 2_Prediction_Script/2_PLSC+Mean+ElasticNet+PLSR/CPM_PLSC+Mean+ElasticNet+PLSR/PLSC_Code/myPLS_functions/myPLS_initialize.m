...
%   check input validity, auto-complete missing fields to ensure smooth subsequent analysis workflow.
%   Supports multiple input formats, grouping, behavior/contrast/interaction analysis,
%   normalization, plotting and save parameters, etc.
%
% =============================
% Inputs:
%   input    : Structure containing analysis input data, must include:
%       .brain_data/X0   : N×M matrix, raw imaging data
%       .behav_data/Y0   : N×B matrix, raw behavior/design data
%       .grouping        : N×1 vector, subject grouping information
%       .group_names     : 1×G cell array, group names (optional)
%       .behav_names     : 1×B cell array, behavior variable names (optional)
%       .img_names       : 1×M cell array, imaging variable names (optional)
...
%       .normalization_behav  : Behavior data normalization method (0~4, see myPLS_bootstrapping.m)
%   save_opts : Structure, save and plot parameters, must include:
%       .prefix        : String, result filename prefix
...
% =============================
% Main program workflow starts
% =============================

disp('... Initializing input parameters ...')

%% ========== 1. Input Data Check and Completion ========== %%
% Support X0 and Y0 input, allowing field names X0/Y0 or brain_data/behav_data in input structure
% Reason: Some users may use X0/Y0 to name input fields, auto-completion is needed for compatibility
if ~isfield(input, 'brain_data') && isfield(input, 'X0')
    % If brain_data is not provided but X0 exists, auto-complete
    input.brain_data = input.X0;
elseif ~isfield(input, 'brain_data') && ~isfield(input, 'X0')
    % Reason: If neither brain_data nor X0 exists, imaging data is missing, cannot proceed with analysis
    error('Missing imaging data input (brain_data or X0 field)');
end

% Reason: Some users may use Y0 to name input fields, auto-completion is needed for compatibility
if ~isfield(input, 'behav_data') && isfield(input, 'Y0')
    % If behav_data is not provided but Y0 exists, auto-complete
    input.behav_data = input.Y0;
elseif ~isfield(input, 'behav_data') && ~isfield(input, 'Y0')
    % Reason: If neither behav_data nor Y0 exists, behavioral data is missing, cannot proceed with analysis
    error('Missing behavioral data input (behav_data or Y0 field)');
end

% Reason: PLS analysis requires consistent sample size for X and Y, otherwise data mismatch occurs
...
%% ========== Group Names and Behavior Variable Names Completion ========== %%
...
if isfield(input, 'group_names') && numel(input.group_names) < nGroups
    ...
    input.group_names = []; % Clear, will be auto-completed later
end

% Reason: If group_names is not provided or is empty, auto-generate standard group names to ensure subsequent workflow availability
...
% Reason: If the number of behav_names is less than the actual number of behavior variables, information is incomplete and needs to be reset to standard naming
if isfield(input, 'behav_names') && numel(input.behav_names) < nBehav
    disp('!!! Number of behavior variable names is less than actual number of variables, will use standard naming automatically!');
    input.behav_names = [];
end

% Reason: If behav_names is not provided or is empty, auto-generate standard behavior variable names to ensure subsequent workflow availability
...
% Reason: If the number of behav_names exceeds the number of behavior variables, it is redundant and needs to be truncated
if numel(input.behav_names) > nBehav
    disp('!!! Number of behavior variable names exceeds actual number of variables, only keeping the first few:');
...
%% ========== 2. PLS Parameter Completion and Check ========== %%
% Reason: If analysis type is not specified, default to behavior PLS to ensure workflow availability
...
elseif ~(strcmp(pls_opts.behav_type, 'behavior') || strcmp(pls_opts.behav_type, 'contrast') || strcmp(pls_opts.behav_type, 'contrastBehav') || strcmp(pls_opts.behav_type, 'contrastBehavInteract'))
    % Reason: If analysis type is not in allowed range, throw error to prevent subsequent errors
    error('Invalid behavior analysis type (behav_type), please check parameters!')
end

% Reason: If PLS grouping is not specified, automatically set based on analysis type to ensure reasonable parameters
if ~isfield(pls_opts, 'grouped_PLS')
    disp('PLS grouping option not specified, will automatically set based on analysis type:')
    if contains(pls_opts.behav_type, 'contrast')
        ...
        disp('   Analysis type is behavior, PLS will be performed by group')
    end
end

% Reason: For contrast PLS, within-group normalization is not allowed, and grouped PLS is not permitted, otherwise the mathematical meaning is inconsistent
if contains(pls_opts.behav_type, 'contrast')
...
% Reason: If permutation test grouping is not specified, automatically set based on analysis type to ensure reasonable parameters
if ~isfield(pls_opts, 'grouped_perm') || isempty(pls_opts.grouped_perm)
    disp('Permutation test grouping option not specified, will automatically set based on analysis type:')
    if contains(pls_opts.behav_type, 'contrast')
        pls_opts.grouped_perm = 0;
        disp('   Analysis type is contrast, permutation test will not consider grouping')
    else
        pls_opts.grouped_perm = 1;
        disp('   Analysis type is behavior, permutation test will be performed by group')
    end
end

% Reason: If bootstrap sampling grouping is not specified, automatically set based on analysis type to ensure reasonable parameters
if ~isfield(pls_opts, 'grouped_boot') || isempty(pls_opts.grouped_boot)
    disp('Bootstrap sampling grouping option not specified, will automatically set based on analysis type:')
    if contains(pls_opts.behav_type, 'contrast')
        ...
        disp('   Analysis type is behavior, bootstrap sampling will be performed by group')
    end
end
...
if pls_opts.grouped_boot ~= pls_opts.grouped_perm
    disp('!!! Permutation test and bootstrap sampling grouping options are inconsistent, please confirm analysis requirements!')
end
if pls_opts.grouped_PLS ~= pls_opts.grouped_perm
    disp('!!! PLS and permutation test grouping options are inconsistent, please confirm analysis requirements!')
end

% Reason: If Procrustes transformation mode is not specified, default to 1 to ensure workflow availability
...
% Reason: Procrustes mode only allows 1 or 2, other values will raise an error
...
% Reason: If not specified whether to save bootstrap samples, automatically set based on number of imaging variables to prevent storage pressure from large data
if ~isfield(pls_opts, 'save_boot_resampling') || isempty(pls_opts.save_boot_resampling)
    disp('Save bootstrap samples option not specified, will automatically set based on number of imaging variables:')
    if nImg > 1000
...
% Reason: If user forces saving but number of variables is too large, warn that it may cause large files
if pls_opts.save_boot_resampling && nImg > 1000
    disp('!!! Save bootstrap samples is selected, but number of imaging variables is greater than 1000, may cause large file size!')
end

disp(' ')

% =============================
% Main program workflow ends
% =============================
