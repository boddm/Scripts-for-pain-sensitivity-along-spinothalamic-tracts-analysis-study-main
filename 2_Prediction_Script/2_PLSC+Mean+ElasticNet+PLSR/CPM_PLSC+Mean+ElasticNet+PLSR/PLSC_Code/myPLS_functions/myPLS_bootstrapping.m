...
% Each bootstrap sample recalculates PLS analysis and outputs various statistics and scores.
% Supports within/between group sampling, Procrustes rotation, saving bootstrap samples, etc.
%
% =============================
% Inputs:
% X0             : N×M matrix, original imaging data (not normalized)
%                  N: number of subjects
%                  M: number of imaging variables
% Y0             : N×B matrix, original behavioral/design data (not normalized)
%                  B: number of behavioral/design variables
% U              : B×L matrix, behavioral/design salience
...
%                         1 = concatenate between-group covariance matrices (traditional behavioral PLS)
%   .grouped_boot        : logical/0 or 1, whether to sample within groups
%                         0 = ignore groups, sample from whole population
...
%                         2 = rotate U and V separately, take average rotation
...
%                         2 = z-score within groups (default, each group separately)
...
%   .normalization_behav : integer, behavioral/design data normalization method
%                         0 = no normalization
%                         1 = z-score across all subjects (mean=0, variance=1)
%                         2 = z-score within groups (default, each group separately)
%                         3 = standard normalization across all subjects (no centering, variance=1)
...
% =============================
% Outputs:
% boot_results : structure, contains all bootstrap results and statistics:
%   .Ub_vect    : B×L×P matrix, bootstrap behavioral salience vectors
%                 B: number of behavioral/design variables, L: number of LCs, P: number of bootstrap samples
...
%   .U_stad     : ratio of U to standard error (U ./ std error)
%   .V_stad     : ratio of V to standard error (V ./ std error)
%
% =============================
% Example:
...
%% ========== Bootstrap sampling main loop ========== %%
disp('... Bootstrap sampling ...') % Display progress

% Calculate bootstrap sampling orders
...
for iB = 1:pls_opts.nBootstraps
    ...
    switch pls_opts.boot_procrustes_mod
        ...
        case 2
            % Rotate U and V separately, then take average
    ...
    %% Store bootstrap behavioral salience vectors and imaging salience vectors in Ub_vect and Vb_vect
    Ub_vect(:, :, iB) = Ub;
    Vb_vect(:, :, iB) = Vb;
...
% Save all statistics fields to boot_results
fN = fieldnames(boot_stats);
for iF = 1:length(fN)
    boot_results.(fN{iF}) = boot_stats.(fN{iF});
end

% Save bootstrap sampling data if required
