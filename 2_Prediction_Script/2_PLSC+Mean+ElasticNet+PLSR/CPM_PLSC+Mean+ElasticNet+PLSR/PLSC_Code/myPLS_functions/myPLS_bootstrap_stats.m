...
% =============================
% Function:
% Calculate statistics (mean, std, confidence intervals) for PLS bootstrap
% resampling of saliences and loadings. Output descriptive statistics of
% all bootstrap distributions for subsequent significance testing and visualization.
%
% =============================
% Inputs:
% Ub_vect      : B×L×P matrix, bootstrap behavioral/design saliences
%                B: number of behavioral/design variables, L: number of latent components, P: number of bootstrap samples
% Vb_vect      : M×L×P matrix, bootstrap imaging saliences
...
%   .Lyb                    : N×(L×n_groups)×P matrix, bootstrap behavioral scores
%   .LC_img_loadings_boot   : M×L×P matrix, bootstrap imaging loadings
%   .LC_behav_loadings_boot : B×(L×n_groups)×P matrix, bootstrap behavioral loadings
%
% =============================
% Outputs:
% boot_stats : structure containing all bootstrap statistics:
%   .Ub_mean, .Ub_std, .Ub_lB, .Ub_uB                       : B×L matrix, behavioral/design salience mean, std, confidence intervals
%   .Vb_mean, .Vb_std, .Vb_lB, .Vb_uB                       : M×L matrix, imaging salience mean, std, confidence intervals
%   .LC_behav_loadings_mean, .LC_behav_loadings_std         : B×(L×n_groups) matrix, behavioral/design loadings mean, std
%   .LC_behav_loadings_lB, .LC_behav_loadings_uB            : B×(L×n_groups) matrix, behavioral/design loadings confidence intervals
%   .LC_img_loadings_mean, .LC_img_loadings_std             : M×L matrix, imaging loadings mean, std
%   .LC_img_loadings_lB, .LC_img_loadings_uB                : M×L matrix, imaging loadings confidence intervals
%
% =============================
% Example:
...
%% ========== Behavioral/Design and Imaging Saliences Statistics ========== %%
% CPM process only uses V_stad, so only Ub/Vb statistics needed for V_stad calculation are retained here.
boot_stats.Ub_mean = mean(Ub_vect, 3); % behavioral/design salience mean
boot_stats.Ub_std = std(Ub_vect, [], 3); % behavioral/design salience standard deviation
...
boot_stats.Vb_mean = mean(Vb_vect, 3); % imaging salience mean
boot_stats.Vb_std = std(Vb_vect,[], 3); % imaging salience standard deviation
...
%% ========== The following statistics are only used for complete PLSC plotting/reporting ========== %%
