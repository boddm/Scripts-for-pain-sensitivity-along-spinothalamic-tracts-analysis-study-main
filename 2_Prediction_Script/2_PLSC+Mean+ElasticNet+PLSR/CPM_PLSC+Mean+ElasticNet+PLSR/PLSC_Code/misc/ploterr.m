...
while idx <= n
    prop = A{idx};
    val = A{idx+1};
    if all(prop(end-1:end) == 'yx') || all(prop(end-1:end) == 'xy')
        prop = prop(1:end-2); 
    end
    ...
    switch prop
     ...
     case 'hhx'
        if isnumeric(val)
            ...
            error('Property hhx must be followed by a numerical value.');
        end
     case 'hhy'
        if isnumeric(val)
            hy = abs(val);
        else 
            error('Property hhy must be followed by a numerical value.');
        end
     case 'hh'
        if isnumeric(val)
            hy = abs(val); 
            ...
            error('Property hh must be followed by a numerical value.');
        end
     case 'abshhx'
